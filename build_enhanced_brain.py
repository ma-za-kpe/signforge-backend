...
def main():
    # Check for brain directory in multiple locations
    if Path("/app/ghsl_brain").exists():
        brain_dir = Path("/app/ghsl_brain")  # Docker container
    elif Path("/output").exists():
        brain_dir = Path("/output")  # Original location
    else:
        brain_dir = Path(__file__).parent.parent / "ghsl_brain"  # Local development
